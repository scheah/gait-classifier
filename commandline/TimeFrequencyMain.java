...
public class TimeFrequencyMain {

    public static void addFilesFromDir(TimeFrequencyClassifier classifier, String directoryPath, String className) {
        ...
        try {
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                ...
                if (listOfFiles[i].toString().contains("TimeFreq")) {
                    reader = new BufferedReader(new FileReader(listOfFiles[i]));
                    classifier.loadGaitData(reader, className, listOfFiles[i].toString());
                    reader.close();
                }
            }
        }
    ...
    public static void processHMPData(int classOption) {
        ...
        String [] userClasses = new String[]{"f1", "f2", "f3", "f4", "f5", "m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9",
            "m10", "m11"};
        TimeFrequencyClassifier m_classifier = null;
        if (classOption == 1) {
            m_classifier = new TimeFrequencyClassifier(actionClasses);
            ...
            m_classifier = new TimeFrequencyClassifier(userClasses);
