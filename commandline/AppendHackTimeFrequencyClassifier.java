 ...
 */
public class AppendHackTimeFrequencyClassifier {
    private static final String TAG = "AppendHackTimeFrequencyClassifier";
    private Instances m_Data;
    private Classifier m_Classifier;
    private String [] m_Classes;

    public AppendHackTimeFrequencyClassifier(String [] classes) {
        m_Classes = classes;
        ...
        while ((mLine = reader.readLine()) != null) {
            ...
            if (mLine.contains(":") || mLine.contains("="))
                ...
                if (rawData.numInstances() != 0) { // skip any blank line if we haven't read in any data yet
                    ...
                    double [] transformed = iDataPoint.toDoubleArray();
                    String textid = filename.substring(filename.length() - 8, filename.length() - 4);
                    String directory = filename.substring(0, filename.lastIndexOf("\\")+1);
                    System.out.println(filename);
                    BufferedReader freqreader = new BufferedReader(new FileReader(directory + "spectral_energy_" + textid + ".txt"));
                    String[] stringFreqData = freqreader.readLine().split("\\s+");
                    double[] freqdata = new double[3];
                    for (int i = 0; i < freqdata.length; i++) {
                        freqdata[i] = Double.parseDouble(stringFreqData[i]);
                    }
                    String finalstring = "";
                    double [] finaldata = (double [])join(transformed, freqdata);
                    for (int i = 0; i < finaldata.length; i++) {
                        finalstring += finaldata[i] + " ";
                    }
                    BufferedWriter bw = null;
                    File file = new File(directory + "TimeFreq_" + textid + ".txt" );
    ...
    private Instances initializeTransformedDataFormat2() {
        ...
        transformedData.setClassIndex(transformedData.numAttributes() - 4);
